...
const isConfigured = () => {
	...
	if (key.startsWith("key_live") || secret.startsWith("secret_live")) {
		console.warn(
			"Refusing to verify: live credentials are configured, but only test credentials are supported.",
		);
		return false;
	}
...
const namesAgree = (officialName, documentName) => {
	...
	return (
		parts.filter((part) => official.includes(part)).length / parts.length >= 0.5
	);
};
...
const verifyAadhaar = async ({ idNumber, name, dob }) => {
	...
	if (!otp) {
		return {
			...
			verified: false,
			remarks:
				"Aadhaar cannot be confirmed automatically — UIDAI requires a one-time password sent to the registered mobile number.",
		};
	}
...
const normalizeIdType = (idType) => {
	...
	if (value.includes("driv") || value.includes("licen"))
...
const UNSUPPORTED_REMARKS = {
	aadhaar:
		"Aadhaar cannot be confirmed automatically — UIDAI requires a one-time password sent to the registered mobile number.",
	driving_licence:
		"Driving licence records are not available for automatic confirmation.",
	...
	unknown:
		"The document type could not be identified, so no government check was possible.",
};
