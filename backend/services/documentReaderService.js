import {
	GoogleGenerativeAI as GenerativeClient,
	SchemaType,
...
const EXTRACTION_SCHEMA = {
	...
	properties: {
		name: {
			...
			description:
				"Full name of the document holder in English. Empty string if unreadable.",
		},
		dob: {
			type: SchemaType.STRING,
			description:
				"Date of birth as DD/MM/YYYY. Use YYYY alone if only a year of birth is printed. Empty string if absent.",
		},
		gender: {
			type: SchemaType.STRING,
		...
		idType: {
			type: SchemaType.STRING,
			description:
				"One of Aadhaar, PAN, Driving Licence, Passport, Voter ID, Unknown.",
		},
		idNumber: {
			type: SchemaType.STRING,
			description:
				"The document number exactly as printed. Empty string if unreadable.",
		},
		isIdDocument: {
			type: SchemaType.BOOLEAN,
			description:
				"True only if the image is a genuine government-issued identity document.",
		},
		rawText: {
			type: SchemaType.STRING,
	...
	try {
		...
		if (!document.isIdDocument) {
			return {
				success: false,
				error:
					"This does not look like a government-issued ID. Please upload a clear photo of your Aadhaar or PAN card.",
			};
		}
		...
		if (!nameReadable || !ageReadable) {
			return {
				success: false,
				error:
					"We could not clearly read the details on this ID. Please upload a sharper, better-quality photo — good lighting, no blur, and all text clearly visible.",
			};
		}
		...
		return {
			...
			retryable: true,
			error:
				"We could not read your document just now. Please try uploading it again in a moment.",
		};
	}
};
