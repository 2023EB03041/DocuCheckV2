...
const chargesFor = (reservation) => {
  ...
  const rate = reservation.pricePerNight || (rooms && nights ? Math.round(subtotal / (rooms * nights)) : 0);
...
export const buildReservationPdf = (reservation) => {
  ...
  field('Room Numbers:', (reservation.roomNumbers || []).join(', ') || 'Assigned at check-in', 20, 55, 90);
  field('Booked By:', reservation.email || '-', 20, 55, 100);

  field('Check-In:', new Date(reservation.checkInDate).toLocaleDateString(), 115, 145, 70);
  field('Check-Out:', new Date(reservation.checkOutDate).toLocaleDateString(), 115, 145, 80);
...
export const downloadReservationPdf = (reservation) => {
  buildReservationPdf(reservation).save(`Lumina_Reservation_${reservation.reservationId}.pdf`);
};
